...
export const logicNodeTypes: NodeTypes = {
  diamond: DiamondNode,
  pill: PillNode,
  process: ProcessNode,
  scope: GroupNode,
...
export const knowledgeNodeTypes: NodeTypes = {
...
export const normPath = (value: string): string =>
...
type LayoutStrategy = (input: LayoutInput) => { nodes: Node[]; edges: Edge[] };

const layoutStrategies: Record<"logic" | "knowledge" | "react", LayoutStrategy> = {
  logic: ({ graph, selectedNodeId, fileContentMap, showCalls }) =>
    computeLogicLayout(graph, selectedNodeId, fileContentMap, showCalls),
  knowledge: ({ graph, selectedNodeId, fileContentMap }) =>
    computeFlatLayout(graph, selectedNodeId, fileContentMap),
  react: ({ graph, selectedNodeId, fileContentMap }) =>
    computeFlatLayout(graph, selectedNodeId, fileContentMap),
};

export const computeLayoutByView = (
  viewType: "logic" | "knowledge" | "react",
  graph: ViewGraph,
  selectedNodeId: string,
  fileContentMap: Map<string, string>,
  showCalls: boolean,
): { nodes: Node[]; edges: Edge[] } => layoutStrategies[viewType]({
  graph,
  selectedNodeId,
  fileContentMap,
  showCalls,
